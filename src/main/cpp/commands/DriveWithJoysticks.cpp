...
DriveWithJoysticks::DriveWithJoysticks(DriveTrain* driveTrain, Intake* intake, std::function<double()> forward, std::function<double()> turn, std::function<bool()> shouldIntake, std::function<bool()> shouldMoveBalls) : m_driveTrain{driveTrain}, m_intake{intake}, m_forward{forward}, m_turn{turn}, m_shouldIntake{shouldIntake}, m_shouldMoveBalls{shouldMoveBalls} {
